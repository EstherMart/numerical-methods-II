...
/**
 * Esta classe implementa métodos de diferenciação numérica (central, para frente, para trás)
 * aplicados à função seno (pois foi o que o professor utilizou em sala). O objetivo é estimar a derivada em um ponto x dado um deltaX.
...
* Como não podemos calcular o limite numericamente (já que h → 0 seria h infinitesimal),
...
public class DerivadaLimite {
    ...
    public static double func(double x) {
        return Math.sin(x); // Pode trocar por Math.cos(x), Math.exp(x), utilizei essa pois estamos trabalhando com a função seno e etc..
    }
     ...
     * @param direction tipo de derivada: 0 (central), 1 (backword - à esquerda), 2 (forward - à direita)
    ...
    /**
     * Interface simples em terminal para interação com o usuário -- a ideia é facilitar na hora de testar...
     */
